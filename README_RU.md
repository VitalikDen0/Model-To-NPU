...
> **Лучший подтверждённый исторический тёплый SDXL-путь (v0.3.0) — ~30 с итого** (UNet ~19 с, VAE ~1.9 с, CLIP ~9 мс кэш)
...
> [!WARNING]
> WAN end-to-end beta сейчас **НЕ ПРОВЕРЕН** и может работать нестабильно или не работать вовсе.
> Hot-swap по `WxH` и HotSwap LoRA находятся в тестовой стадии и могут ломаться.
> Цель на стабилизацию/полировку после `v0.5.0`: около **2 недель**.
...
- **APK `v0.5.0`:** SDXL и WAN разделены на отдельные вкладки, настройки генерации сохраняются отдельно по вкладкам
...
**Обновление roadmap (v0.4.8-beta3):** SDXL временно заморожен как продуктовая линия, основной фокус разработки смещён на **WAN** и **FLUX**. SD1.5 и SD3.5 рассматриваются как **обучающие стенды разработчика** — для понимания/отладки методов, но не как обязательные deliverables.
...
### v0.4.8-beta3 — фиксы скорости генерации + известный остаточный хвост

Для `qnn-multi-context-server` усилен HTP perf-mode (DCVS отключён, bus/core corners закреплены в MAX, добавлены RPC latency/polling controls). В локальной валидации это снизило latency decoder с класса около `~820 мс` до примерно `~725–776 мс`.

При этом остаётся остаточный хвост порядка `~50 мс` относительно исторического идеального маркера `v0.4.7` в текущем окружении. После прямого сравнения код/путь причина полностью не подтверждена, поэтому в релизе это зафиксировано явно.

### Текущая линия APK (v0.4.7) — stability-first refresh + CFG/TAESD hotfixes

Текущая публичная линия `v0.4.7` сохраняет stability-first решения из `v0.4.6`: в публичном APK по-прежнему отключён app-open background prewarm, foreground-run больше не просит phone runtime агрессивно прогревать все контексты / preview assets, а маркер packaged runtime payload всё так же строится из fingerprint реально staged payload, поэтому обновлённые `generate.py` / bundled QNN assets надёжно переэкстрагируются на устройстве вместо молчаливого повторного использования устаревших распакованных файлов. Поверх этой базы в приложении теперь всегда прокидывается точное пользовательское значение CFG, включая **`1.0`** (то есть `CFG=1.0` больше не сваливается молча обратно на runtime default `3.5`), а сбои TAESD/live preview явно показываются в APK UI как **нефатальные предупреждения**, пока сама генерация продолжает работать. Линия по-прежнему опирается на текущий реальный cold-start on-device proof shot на **34.6 с total**. Внутри этого прогона accelerator-visible стадии суммарно дают **~16.25 с** (`CLIP 0.134 с`, `UNet 14.248 с`, `VAE 1.872 с`), а показанное на скриншоте итоговое время всё ещё включает cold start / runtime bring-up; в текущем быстром пути наблюдаемые fast-path температуры держались примерно в диапазоне **85–95°C** без видимого троттлинга на нескольких коротких подряд прогонах. В этой сессии для `v0.4.7` также успешно прошли свежие локальные сборки **debug** и **release** APK.
...
Подробные исторические данные и архивные заметки по `0.3.x` / `0.2.x` — см. [HISTORY_RU.md](HISTORY_RU.md).
