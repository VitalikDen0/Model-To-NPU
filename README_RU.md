...
### Текущая линия APK (v0.4.6) — stability-first refresh

Текущая публичная линия `v0.4.6` делает ставку на стабильность, а не на спекулятивный warm-state reuse: в публичном APK отключён app-open background prewarm, foreground-run больше не просит phone runtime агрессивно прогревать все контексты / preview assets, а маркер packaged runtime payload теперь строится из fingerprint реально staged payload, поэтому обновлённые `generate.py` / bundled QNN assets надёжно переэкстрагируются на устройстве вместо молчаливого повторного использования устаревших распакованных файлов. Обновлённая линия теперь уже повторно подтверждена и реальным cold-start on-device proof shot на **34.6 с total**. Внутри этого прогона accelerator-visible стадии суммарно дают **~16.25 с** (`CLIP 0.134 с`, `UNet 14.248 с`, `VAE 1.872 с`), а показанное на скриншоте итоговое время всё ещё включает cold start / runtime bring-up; в текущем быстром пути наблюдаемые fast-path температуры держались примерно в диапазоне **85–95°C** без видимого троттлинга на нескольких коротких подряд прогонах.
