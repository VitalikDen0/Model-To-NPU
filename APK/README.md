...
Текущая документированная версия APK: **`0.2.1`**.
...
- APK `v0.2.1` перед запуском runtime явно экспортирует `SDXL_QNN_USE_MMAP=1`, `SDXL_QNN_PERF_PROFILE=sustained_high_performance`, включает live-температуры, автоматически добавляет `SDXL_QNN_CONFIG_FILE`, если рядом лежат `htp_backend_extensions_lightning.json` и `lib/libQnnHtpNetRunExtensions.so`, и уводит временные `WORK_DIR` / preview / output-файлы в app cache, чтобы меньше упираться в shared storage
