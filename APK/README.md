...
**v0.1.1** (текущая):
- Чекбокс **Live Preview (TAESD)** — показывает промежуточные шаги денойзинга прямо во время генерации
- Требует задеплоенного `taesd_decoder.serialized.bin.bin`
...
- **Live Preview (TAESD)** — промежуточные preview во время денойзинга, если задеплоен TAESD context
- **Сохранение** — в галерею
- **Stop** — прерывание генерации
- **Прогресс-бар** — этапы CLIP -> UNet -> VAE
- **⚙️ Settings** — путь к моделям и Python

> **Важно:** текущий APK и все публичные примеры в этом репозитории рассчитаны на **1024×1024**. Показанные тайминги и примеры также предполагают, что **Lightning LoRA уже замержена в UNet**; без этого merge базовый SDXL путь получается существенно медленнее.
...
- `phone_generate.py` уже умеет `--prog-cfg`, но текущий APK UI в `v0.1.1` выставляет именно `--preview`, а не отдельный переключатель Progressive CFG
...
- TAESD нужен только для preview-декодирования промежуточных latent'ов и не заменяет основной VAE decode финальной картинки
