...
Текущая документированная версия APK: **`0.5.1`**.

### v0.5.1 — Hot Preload, LoRA UI, Wan UI Fixes

- добавлена кнопка **Preload**, которая подгружает SDXL модель в память (запускает background-сервер на 30 сек), освобождая интерфейс;
- добавлен выпадающий список **LoRA slot** для быстрой смены слотов (none, slot1, slot2 и т.д.);
- создана заготовка скрипта `SDXL/build_lora_slot.py` для host-side создания LoRA-контекстов;
- в интерфейсе Wan 2.1 добавлены слайдеры **Frames** и **FPS**, а дефолтные шаги изменены на 30;
- исправлены баги со сбросом ручного ввода в поля WxH (теперь работает корректный snap).

### v0.5.0 — SDXL/WAN split tabs + beta guardrails

- главный экран разделён на две вкладки: **SDXL** и **WAN 2.1**;
- настройки генерации теперь сохраняются **отдельно для каждой вкладки** (prompt/seed/steps/cfg/WxH и т.д.);
- WAN path по-прежнему остаётся beta/runtime-oriented и не считается подтверждённым production-flow;
- добавлен отдельный WAN end-to-end helper `WAN 2.1 1.3B/run_end_to_end.ps1`.

### v0.4.8-beta3 — speed-fix release (HTP perf tuning)

- для `qnn-multi-context-server` усилен perf-профиль исполнения (DCVS off, MAX corners, RPC latency/polling controls);
- локально decoder latency улучшился примерно с `~820 мс` до диапазона `~725–776 мс`;
- остаточный хвост около `~50 мс` относительно исторического `v0.4.7` ideal path пока не объяснён полностью и зафиксирован как known issue релиза.

**Roadmap note:** SDXL временно заморожен как продуктовая ветка; приоритет разработки смещён в **WAN** и **FLUX**. SD1.5/SD3.5 остаются как обучающие стенды для разработчика (не обязательные deliverables).

### v0.4.8-beta3 — Диагностика и улучшение доступа к файлам

- добавлено предупреждение о legacy base-dir redirect: отображается как inline-статус в стиле TAESD, когда `ROOT_BASE_DIR` отличается от настроенного `BASE_DIR`;
- улучшена инициализация no-root пути: теперь при первом запуске автоматически открывается экран **«Разрешение на все файлы»** для `/sdcard/Download/sdxl_qnn`;
- добавлены debug-записи `runtimePlan`/`runtimeEnv` в лог для диагностики QNN окружения.

### v0.4.8-beta2 — Runtime bug fix и копирование ошибки

- исправлен runtime bug в линии APK, который мог приводить к неуспешному старту/падению генерации в части конфигураций;
- добавлена нижняя кнопка **«Скопировать ошибку»**, которая появляется только в error-state и копирует полный текст ошибки в буфер обмена;
- усилена обработка невалидного `seed` и других error-path в UI, чтобы уменьшить crash-like сценарии.

### v0.4.8-beta — Bundled Python runtime, no-root путь, двойные пути в Настройках

- APK теперь поставляется со **встроенным Python 3.13 runtime** (`py_runtime`) внутри assets — Termux больше не нужен для генерации на поддерживаемых устройствах;
- двойные пути в Настройках: **no-root путь** (`/sdcard/Download/sdxl_qnn`) и **root путь** (`/data/local/tmp/sdxl_qnn`) настраиваются отдельно; при запуске приложение автоматически выбирает root путь, если он доступен, иначе — no-root путь;
- **TAESD live preview намеренно отключён в этой beta**: загрузка TAESD HTP контекста в общий `_QnnMultiContextServer` давала ~200 мс оверхед на каждый шаг UNet из-за конкуренции за DSP память; GPU путь (`libQnnGpu.so`) не работает в app-процессе из-за ограничений Android linker namespace; отдельный GPU backend находится в разработке и будет включён в следующем релизе;
- `taesd_htp.bin` остаётся в bundled payload для forward compatibility, но backend выставлен в `off`.
...
- APK `v0.4.7` оставляет generation-path на валидированных preset-ориентирах и сохраняет screen-sized decode path для preview/final PNG, чтобы UI не тащил лишние full-resolution bitmap'ы
- APK `v0.4.7` полностью отключает app-open background prewarm в публичной линии, чтобы QNN server / контексты не сидели в памяти телефона ещё до реального запуска генерации
- APK `v0.4.7` оставляет APK-side `SDXL_QNN_PERF_PROFILE` на `burst`, но больше не просит runtime агрессивно prewarm-ить все контексты и preview assets в foreground-run
- APK `v0.4.7` пишет content-derived marker в `runtime_payload_version.txt`, поэтому новые bundled `generate.py` / QNN runtime assets действительно заменяют старые распакованные on-device копии
- APK `v0.4.7` теперь всегда прокидывает в phone runtime точное значение `--cfg`, включая `1.0`, поэтому ручной no-guidance режим больше не сваливается обратно на Python default `3.5`
- APK `v0.4.7` парсит структурированные `TAESD_WARNING:`-линии из runtime и явно показывает нефатальную проблему live preview в статусе/таймингах вместо молчаливого отсутствия preview
