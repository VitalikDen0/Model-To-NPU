...
Текущая документированная версия APK: **`0.1.4`**.
...
Лучший session-validated путь для `v0.1.4` с `sustained_high_performance`, backend extensions и `--prog-cfg` (`8` шагов, `CFG=3.5`) дал **79.7–80.6 s total** на OnePlus 13. При этом в полном прогоне live-лог температуры обычно держался около **CPU ~59–70°C**, **GPU ~50–52°C**, **NPU ~57–72°C**, с короткими NPU-пиками примерно до **78°C**.
...
- APK `v0.1.4` перед запуском runtime явно экспортирует `SDXL_QNN_USE_MMAP=1`, `SDXL_QNN_PERF_PROFILE=sustained_high_performance`, включает live-температуры и автоматически добавляет `SDXL_QNN_CONFIG_FILE`, если рядом лежат `htp_backend_extensions_lightning.json` и `lib/libQnnHtpNetRunExtensions.so`
